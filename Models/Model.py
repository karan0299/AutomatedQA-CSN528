...
class QAModel(object):
    ...
    def add_placeholders(self):
        ...
        self.qn_ids = tf.placeholder(tf.int32, shape=[None, question_len])
        self.qn_mask = tf.placeholder(tf.int32, shape=[None, question_len])
    ...
    def build_graph(self):
        ...
        ###################################################### RNN ENCODER  #######################################
        encoder = RNNEncoder(hidden_size_encoder, self.keep_prob)
       ...
       ###################################################### BASELINE FULLY CONNECTED ANSWER PREDICTION  #######################################
        # Use baseline fully connected for answer start and end prediction

        # Apply fully connected layer to each blended representation
        # Note, blended_reps_final corresponds to b' in the handout
        # Note, tf.contrib.layers.fully_connected applies a ReLU non-linarity here by default
        blended_reps_final = tf.contrib.layers.fully_connected(blended_reps, num_outputs=hidden_size_fully_connected) # blended_reps_final is shape (batch_size, context_len, hidden_size)
    ...
    def run_train_iter(self, session, batch, summary_writer):
        ...
        # print(type(batch.context_ids))
        # print(type(batch.context_mask))
        # print(batch.qn_ids)
        # print(type(batch.qn_mask))
    ...
    def get_dev_loss(self, session, dev_context_path, dev_qn_path, dev_ans_path):
        ...
        for batch in get_batch_generator(self.word2id, dev_context_path, dev_qn_path, dev_ans_path, batch_size, context_len=context_len, question_len=question_len, discard_long=True):
    ...
    def check_f1_em(self, session, context_path, qn_path, ans_path, dataset, num_samples=100, print_to_screen=False):
        ...
        for batch in get_batch_generator(self.word2id, context_path, qn_path, ans_path, batch_size, context_len=context_len, question_len=question_len, discard_long=False):
    ...
    def train(self, train_dir ,session, train_context_path, train_qn_path, train_ans_path, dev_qn_path, dev_context_path, dev_ans_path):
        """
        Main training loop.
        ...
        checkpoint_path = train_dir + "qa.ckpt"
        bestmodel_dir = train_dir + "best_checkpoint/"
        bestmodel_ckpt_path = bestmodel_dir + "qa_best.ckpt"
        best_dev_f1 = None
        best_dev_em = None

        # for TensorBoard
        summary_writer = tf.compat.v1.summary.FileWriter(train_dir, session.graph)
